# This file is responsible for configuring your application
# and its dependencies with the aid of the Config module.
#
# This configuration file is loaded before any dependency and
# is restricted to this project.

# General application configuration
import Config

# Configures the endpoint
config :phx_notifications, PhxNotificationsWeb.Endpoint,
  url: [host: "localhost"],
  render_errors: [
    formats: [html: PhxNotificationsWeb.ErrorHTML, json: PhxNotificationsWeb.ErrorJSON],
    layout: false
  ],
  pubsub_server: PhxNotifications.PubSub,
  live_view: [signing_salt: "d9rsQxDE"]

# Configure esbuild (the version is required)
config :esbuild,
  version: "0.17.11",
  default: [
    args:
      ~w(js/app.js --bundle --target=es2017 --outdir=../priv/static/assets --external:/fonts/* --external:/images/* --define:process.env.PUSH_PUBLIC_KEY='#{System.get_env("PUSH_PUBLIC_KEY")}'),
    cd: Path.expand("../assets", __DIR__),
    env: %{"NODE_PATH" => Path.expand("../deps", __DIR__)}
  ],
  # Need to put service_worker.js to app-root directory to work.
  sw: [
    args:
      ~w(js/service_worker.js --target=es2017 --outfile=../priv/static/sw.js),
    cd: Path.expand("../assets", __DIR__),
    env: %{"NODE_PATH" => Path.expand("../deps", __DIR__)}
  ]

# Configure tailwind (the version is required)
config :tailwind,
  version: "3.3.2",
  default: [
    args: ~w(
      --config=tailwind.config.js
      --input=css/app.css
      --output=../priv/static/assets/app.css
    ),
    cd: Path.expand("../assets", __DIR__)
  ]

# Configure Web push
config :web_push_encryption, :vapid_details,
  subject: System.get_env("PUSH_SUBJECT"),
  public_key: System.get_env("PUSH_PUBLIC_KEY"),
  private_key: System.get_env("PUSH_PRIVATE_KEY")

config :web_push_encryption, :http_client, PhxNotifications.HttpClient

# Configures Elixir's Logger
config :logger, :console,
  format: "$time $metadata[$level] $message\n",
  metadata: [:request_id]

# Use Jason for JSON parsing in Phoenix
config :phoenix, :json_library, Jason

# Import environment specific config. This must remain at the bottom
# of this file so it overrides the configuration defined above.
import_config "#{config_env()}.exs"
